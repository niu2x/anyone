...
{
    if (!is_ipv6(s, e))
        throw Url::parse_error("IPv6 [" + std::string(s, e - s)
                               + "] is invalid");
    if ((e - s) == 2 && s[0] == ':' && s[1] == ':')
...
void Url::parse_url() const
{
    ...
    if (p == e) {
        ...
        if (p != e && *p == '/' && (e - b) > 1 && b[0] == '/' && b[1] == '/') {
            const char* ea = find_first_of(
                b += 2, e, "/?#"); // locate end of authority
            p = find_char(b, ea, '@');
            // get user info if any
            if (p != ea) {
                if (!is_chars(b, p, 0x25))
                    throw Url::parse_error("User info in '"
                                           + std::string(s, e - s)
                                           + "' is invalid");
                user_b = b;
                user_e = p;
            ...
            if (*b == '[') {
                ...
                p = find_char(++b, ea, ']');
                if (*p != ']')
                    throw Url::parse_error("Missing ] in '"
                                           + std::string(s, e - s) + "'");
                ...
                if (*b == 'v') {
                    ...
                    if (ip_v == -1 || *b++ != '.' || !is_chars(b, p, 0x05))
                        throw Url::parse_error("Host address in '"
                                               + std::string(s, e - s)
                                               + "' is invalid");
                } else if (is_ipv6(b, p)) {
                    ip_v = 6;
            ...
            if (b != ea && *b == ':') {
                if (!is_port(++b, ea))
                    throw Url::parse_error("Port '" + std::string(b, ea - b)
                                           + "' in '" + std::string(s, e - s)
                                           + "' is invalid");
                port_b = b;
                port_e = ea;
        ...
        if (p != e && *p == '#') {
            if (!is_chars(p + 1, e, 0x3F))
                throw Url::parse_error("Fragment '" + std::string(p + 1, e)
                                       + "' in '" + std::string(s, e - s)
                                       + "' is invalid");
            fragment_b = p + 1;
            fragment_e = e;
    ...
    if (query_b) {
        ...
        while (p != query_e) {
            ...
            if (!is_chars(b, p, 0x3F))
                throw Url::parse_error("Query key '" + std::string(b, p)
                                       + "' in '" + std::string(s, e - s)
                                       + "' is invalid");
